...
const Page = ({ params }) => {
  ...
  async function fetchAnimeData() {
    ...
    try {
      const response = await fetch(
        `https://api.jikan.moe/v4/anime/${params.anime_id}/full`
      );
  ...
  async function fetchCharacters() {
    ...
    try {
      const response = await fetch(
        `https://api.jikan.moe/v4/anime/${params.anime_id}/characters`
      );
      if (!response.ok) {
        throw new Error(`An error has occured`);
      }
  ...
  async function getRecommended() {
    try {
      const resp = await fetch(
        `https://api.jikan.moe/v4/anime/${params.anime_id}/recommendations`
      );
      if (!resp.ok) {
  ...
  return (
    <div className="mt-[300px] mx-5 md:mx-0 md:mt-[150px]">
      {animeData.error === true ? (
        <p className=" text-center mt-10 text-orange-700 font-bold text-3xl h-[20rem]">
        ...
        animeData.data && (
          <main className=" flex flex-col items-center gap-5">
            <h1 className="text-purple-700 text-2xl font-bold text-center md:text-3xl md:w-[65rem] md:leading-9">
            ...
            <div className=" flex flex-wrap justify-center mt-10 gap-5 ">
              ...
              <div className="flex flex-col font-normal mt-2 gap-5 md:font-semibold">
                ...
                {animeData.data.status === "Finished Airing" ? (
                  <Link
                    className="mt-5"
                    href={`/anime/${params.anime_id}/episods`}
                  >
                    <button className="border border-transparent bg-purple-800 text-white font-bold text-xl px-5 py-2 rounded-[5px] hover:translate-x-1 duration-200">
                      Episods
                    </button>
                  </Link>
                ) : (
                  <p className=" text-3xl mt-5 font-bold text-purple-900">
            ...
            <div className="flex flex-col w-[400px] md:w-[51rem]">
              <div className=" text-white mt-5 flex flex-wrap gap-5">
                ...
                <div className="flex flex-wrap self-center gap-5">
                  {animeData.data.genres.map((element) => (
                    <p
                      className={`text-white border font-bold border-transparent bg-orange-700 px-3 py-1 rounded-[25px] ${
                        element.name === "Action"
                          ? "bg-yellow-800"
                          : element.name === "Drama"
                            ? "bg-red-800"
                            : element.name === "Fantasy"
                              ? "bg-blue-900"
                              : "bg-orange-500"
                      }`}
                      key={element.mal_id}
              ...
              <div className=" text-white flex flex-wrap mt-5 gap-5">
                ...
                <div className="flex flex-wrap self-center gap-5">
                  {animeData.data.studios.map((element) => (
                    <p
                      className="text-white border font-bold border-transparent bg-red-700 px-3 py-1 rounded-[25px]"
                      key={element.mal_id}
                    >
                      {element.name}
                    </p>
              ...
              <div className="flex flex-col mt-10 gap-3">
                <h1 className="text-purple-600 text-4xl underline underline-offset-[10px]">
                  Synopsis
                </h1>
              ...
              <div className="flex flex-col mt-10 gap-3">
                {animeData.data.trailer.embed_url ? (
                  <h1 className="text-purple-600 underline underline-offset-8 text-4xl">
                    Trailer
                  </h1>
                ) : (
                  ""
                ...
                {animeData.data.trailer.embed_url ? (
                  ...
                  <p className="text-center mt-10 font-bold text-blue-700 w-auto text-xl md:w-[55rem] md:text-3xl md:mx-0">
              ...
              {characters.error === true ? (
                <p className=" text-2xl text-center mt-10 text-red-800 md:text-4xl">
                  Something went wrong while fetching the charaters
                </p>
              ) : characters.loading === true ? (
                <p className="text-xl text-center font-bold mt-10 text-orange-700 md:text-4xl">
                  ...
                  <div className=" flex flex-col mt-[80px] items-center gap-5">
                    ...
                    <div className="flex flex-wrap gap-5 justify-center md:w-[55rem]">
                      {characters.data.slice(0, more).map((chrt) => (
                        ...
                        >
                          <div
                            className={`flex flex-col gap-1 border border-slate-500 rounded-[5px] p-2 ${
                              chrt.role === "Main"
                                ? "bg-[#070736]"
                                : "bg-slate-800"
                            }`}
                          >
                            ...
                            />
                            <p className="text-white font-semibold w-[150px]">
                              {chrt.character.name}
                            </p>
                            <p
                              className={`${
            ...
            {recommended.length > 0 ? (
              <div className=" mt-5 flex flex-col gap-10">
                <div className=" flex flex-col gap-2">
                  <div className=" flex flex-col gap-2">
                    <p className=" text-purple-900 text-xl font-extrabold md:text-4xl">
                      Related Anime...
                    </p>
                ...
                <div className=" flex flex-row w-[400px] pb-5 overflow-hidden overflow-x-scroll gap-5 scrollbar-custom md:w-[60rem] ">
                  {recommended &&
